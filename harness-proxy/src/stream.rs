...
pub async fn stream(
    ...
    oai.stream_options = Some(crate::openai::StreamOptions {
        include_usage: true,
    });
    ...
    'read: loop {
        ...
        while let Some(nl) = buf.iter().position(|b| *b == b'\n') {
            ...
            let line = match std::str::from_utf8(&line) {
                Ok(line) => line,
                Err(_) => return send_error(&tx, "malformed upstream stream encoding").await,
            };
...
impl Translator {
        ...
        for choice in c.choices {
            ...
            if self.thinking {
                if let Some(r) = d.reasoning.filter(|r| !r.is_empty()) {
                    if !matches!(self.open, Open::Thinking(_)) {
                        ...
                        let i = self.open_block(&mut out, json!({"type":"thinking","thinking":""}));
            ...
            if let Some(text) = d.content.filter(|t| !t.is_empty()) {
                if !matches!(self.open, Open::Text(_)) {
                    self.close_tools(&mut out);
                    let i = self.open_block(&mut out, json!({"type":"text","text":""}));
                    ...
                    *i
                    ...
                    out.push((
                        ...
                        json!({
                            ...
                            "content_block":{
                                "type": "tool_use",
                                "id": tc.id.clone().unwrap_or_else(|| format!("toolu_proxy_{}", tc.index)),
                                "name": tc.function.name.clone().unwrap_or_default(),
                                "input": {}
                            }
                        }),
                    ));
                    self.tools.insert(tc.index, i);
                    i
                };
    ...
    fn open_block(&mut self, out: &mut Vec<(&'static str, Value)>, block: Value) -> i32 {
        self.close_open(out);
        let i = self.next_index;
        self.next_index += 1;
        out.push((
    ...
    fn close_open(&mut self, out: &mut Vec<(&'static str, Value)>) {
        match self.open {
            ...
            Open::Thinking(i) => {
                ...
                out.push(delta(
                    i,
                    json!({"type":"signature_delta","signature":SYNTHETIC_SIGNATURE}),
                ));
                out.push((
                    "content_block_stop",
    ...
    fn close_tools(&mut self, out: &mut Vec<(&'static str, Value)>) {
        for (_, i) in std::mem::take(&mut self.tools) {
            out.push((
                "content_block_stop",
                json!({"type":"content_block_stop","index":i}),
            ));
...
mod tests {
    ...
    #[test]
    fn interleaved_tool_call_indexes_keep_stable_blocks() {
        let (names, payloads) = run(
            false,
            vec![
                json!({"id":"abc","choices":[{"delta":{"role":"assistant"}}]}),
        ...
        let starts: Vec<_> = payloads
            .iter()
            .filter(|p| p["type"] == "content_block_start")
            .collect();
        assert_eq!(starts[0]["content_block"]["id"], "call_0");
        assert_eq!(starts[0]["content_block"]["name"], "first");
        assert_eq!(starts[1]["content_block"]["id"], "call_1");
        assert_eq!(starts[1]["content_block"]["name"], "second");

        let deltas: Vec<_> = payloads
            .iter()
            .filter(|p| p["delta"]["type"] == "input_json_delta")
            .collect();
        assert_eq!(deltas.iter().filter(|p| p["index"] == 0).count(), 2);
        assert_eq!(deltas.iter().filter(|p| p["index"] == 1).count(), 2);
        assert_eq!(
